...
\textbf{Rendering with 3D Gaussian Splatting.}
To achieve photorealistic ground-level rendering without
Blender's offline path tracer, we adopt 3D Gaussian Splatting
(3DGS)~\cite{kerbl2023gaussian} as our primary renderer for
Phase~2. Real-world scenes are captured as \texttt{.ply} point
cloud files in which each Gaussian primitive is parameterized
by center $\boldsymbol{\mu}\!\in\!\mathbb{R}^3$, anisotropic
covariance $\mathbf{\Sigma}$, opacity $\alpha$, and degree-0
spherical harmonic coefficients encoding view-independent
color. At each camera pose along the trajectory the
\texttt{gsplat}~\cite{ye2024gsplat} CUDA rasterizer
alpha-composites the visible Gaussians onto a
$256\!\times\!256$ sensor, producing photorealistic frames at
interactive speeds. Camera intrinsics are fixed ($f\!=\!200$\,px,
principal point at image center). Four scenes covering distinct
ground-surface appearances---grass, tile, long-tile, and
rubber---are used across training and evaluation splits
(Fig.~\ref{fig:textures}), providing visual diversity critical
for cross-texture generalization.
...
\begin{tabular}{cc}
  \includegraphics[width=0.46\columnwidth]{splat_img_and_results/grass.png}   &
  \includegraphics[width=0.46\columnwidth]{splat_img_and_results/tile.png}    \\[2pt]
  {\small Grass} & {\small Tile}                                \\[6pt]
  \includegraphics[width=0.46\columnwidth]{splat_img_and_results/rubber.png}  &
  \includegraphics[width=0.46\columnwidth]{splat_img_and_results/wood.png}    \\[2pt]
  {\small Rubber} & {\small Wood}                               \\
\end{tabular}
\caption{Sample rendered frames from the four Gaussian Splat
scenes used for training and evaluation. Each scene is rendered
from an identical figure-8 trajectory, isolating texture as
the sole variable for generalization assessment.}
\label{fig:textures}
\end{figure}

\textbf{Trajectory Generation.} Four trajectory families---figure-8, spiral, Lissajous, and linear---with smooth 6-DoF motion over 20 seconds (Fig.~\ref{fig:traj_types}). Camera poses are sampled at 10~Hz (200 frames/sequence).
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{visualizations/fig6_texture_quality.png}
    \caption{Original dataset quality: top row shows unusable sequences (blur $<$ 100), bottom row shows usable sequences. 22\% of training data was essentially blank.}
...
\subsection{Qualitative Inference: Per-Modality Trajectories}

Fig.~\ref{fig:inference} shows predicted versus ground-truth
trajectories from each trained model on a held-out evaluation
sequence rendered from the long-tile Gaussian Splat scene.

\textbf{DeepIO} (IMU-only) produces a smooth but rigidly
biased trajectory. The 2-layer GRU regresses to the
conditional mean of the training distribution (figure-8 motion),
accumulating a systematic $-1.43$\,mm/frame bias along the
$x$-axis that compounds to $\sim$2\,m final drift despite
near-perfect per-frame rotation accuracy ($<0.01^\circ$).
This reflects dead-reckoning error accumulation rather than
model failure: any purely relative estimator without
loop-closure or absolute anchors will drift.

\textbf{DeepVO} (vision-only) tracks the qualitative shape
of the trajectory but with scale error and lateral drift.
The CNN encodes texture-specific appearance features rather
than pure optical flow, causing systematic misprediction on
scenes whose texture statistics differ from training.
Per-frame translation error is 75\% of the actual
displacement magnitude---large despite the $<1^\circ$
rotation accuracy---producing $\sim$10\,m trajectory drift.

\textbf{DeepVIO} (FiLM two-stage fusion) achieves the
closest agreement with ground truth. The balanced soft gate
($\alpha\!=\!0.39$--$0.47$) ensures neither modality
dominates: the IMU branch corrects the visual scale error
while the visual branch anchors the inertial drift.
...
\end{tabular}
\caption{Predicted trajectories (dashed) vs.\ ground truth
(solid) on a held-out long-tile Gaussian Splat sequence.
(a)~DeepIO drifts systematically due to dead-reckoning bias
accumulation. (b)~DeepVO tracks circular trajectory shape but exhibits
scale error from texture-dependent feature learning.
(c)~DeepVIO FiLM (two-stage) achieves the best fidelity
through balanced visual-inertial fusion ($\alpha\!\approx\!0.43$).}
\label{fig:inference}
\end{figure}
...
Several promising research directions emerge from our findings for both classical and deep VIO:

\textbf{Classical VIO.} (1)~\textit{Adaptive feature selection}: Classical pipelines (MSCKF, VINS-Mono) use fixed feature detectors that fail in textureless or repetitive environments---learning when to trust visual features vs.\ IMU could improve robustness. (2)~\textit{Tighter filter--learning integration}: Injecting learned noise covariances into EKF/UKF frameworks could combine the geometric rigor of classical filters with data-driven uncertainty estimation. (3)~\textit{Loop closure for deep odometry}: Our models accumulate drift without any mechanism for revisit detection; integrating learned place recognition with classical pose-graph optimization is an open problem.

\textbf{Deep VIO.} (1)~\textit{Motion-invariant representations}: Our models learn trajectory-shape priors (ATE degrades 20$\times$ on unseen linear trajectories). Using optical flow~\cite{teed2020} or egomotion-equivariant features instead of raw images could decouple appearance from geometry. (2)~\textit{Temporal modeling}: Our per-pair prediction discards temporal context. Adding recurrent or transformer-based sequence modeling could capture long-range dependencies and reduce drift. (3)~\textit{Uncertainty-aware fusion}: Replacing the learned soft gate with heteroscedastic uncertainty heads that output per-sample confidence could enable dynamic sensor weighting that generalizes to degraded conditions (e.g., darkness, high vibration). (4)~\textit{Sim-to-real transfer}: While 3D Gaussian Splatting closes the visual domain gap, IMU noise models remain synthetic. Domain randomization over broader noise profiles and cross-domain evaluation on EuRoC~\cite{burri2016} or KITTI~\cite{geiger2012} would validate real-world applicability. (5)~\textit{Cross-modal self-supervision}: Using photometric consistency or IMU preintegration constraints as self-supervised losses could reduce reliance on ground-truth poses, enabling training on unlabeled real-world data.
...
\textit{ACM Trans.\ Graph.}, vol.~42, no.~4, 2023.

\bibitem{ye2024gsplat}
T.~Ye \textit{et al.},
``gsplat: An Open-Source Library for Gaussian Splatting,''
\textit{arXiv preprint arXiv:2409.06765}, 2024.
